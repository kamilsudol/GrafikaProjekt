...
void GUIMyFrame1::gamma_correction_scroll( wxScrollEvent& event )
{
	if (result.data) {
		...
		if (val == slid_size / 2) {
			result = correctGamma(image, 1.);
		}
		else if (val > slid_size / 2) {
			result = correctGamma(image, (gamma_correction_slider->GetMax() - val) / (slid_size / 2.));
		}
		else {
			result = correctGamma(image, pow((gamma_correction_slider->GetMax()/2. - val) / (slid_size/10.) + 1.,2));
		}
...
void GUIMyFrame1::histogram_fun(wxScrolledWindow* histogram, wxImage Img) {
	...
	generate_hist_img(imgHistogramRGB, bitMapHistogramRGB, rgb_count, 0, 0, 0);
